...
// Install display calculation
const calculateDisplayInstalls = (playstoreInstallsStr) => {
  if (!playstoreInstallsStr || typeof playstoreInstallsStr !== 'string') {
    console.warn('Invalid or missing installs string, returning "Unknown"');
    return 'Unknown';
  }
  try {
    const numStr = playstoreInstallsStr.replace(/,/g, '').replace(/\+/g, '');
    const num = parseInt(numStr, 10);
    if (isNaN(num)) {
      console.warn('Invalid installs number format, returning "Unknown"');
      return 'Unknown';
    }
    const percent = Math.floor(Math.random() * 11) + 30;
    const result = (percent / 100.0) * num;
    ...
    console.warn(`Error in calculateDisplayInstalls: ${error.message}, returning "Unknown"`);
    return 'Unknown';
  }
};
...
async function fetchComments(appId) {
  try {
    ...
    const comments = response.data.map(review => ({
      text: review.text || 'No comment text available',
      username: generateUsername(),
      date: randomDate().toISOString()
    }));
    return comments;
      ...
      if (needsUpdate.length > 0) {
        ...
        for (let k = 0; k < needsUpdate.length; k++) {
          ...
          if (isInsert) {
            const newApp = {
              slug: generateSlug(appData.title),
              os: appData.androidVersion || 'Unknown',
              downloadObb: app.downloadObb || false,
              isAvailableOnPlayStore: app.isAvailableOnPlayStore || true,
              appInstall: calculateDisplayInstalls(appData.installs),
              appSize: finalAppSize,
              appId: appData.appId,
              title: appData.title,
              rating: appData.scoreText || 'Unknown',
              ...
              developerURL: generateSlug(appData.developer || 'Unknown'),
              summary: appData.summary || '',
              recentChanges: appData.recentChanges || '',
              category: appData.genre || 'Unknown',
              categoryURL: appData.genreId ? appData.genreId.toLowerCase().replace(/_/g, '-') : 'unknown',
              description: appData.description || '',
              icon: appData.icon || '',
              screenshots: appData.screenshots || [],
              url: appData.url || '',
